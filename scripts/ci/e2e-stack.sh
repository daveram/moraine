...
EOF
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${codex_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${claude_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${routed_clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${claude_keyword}') > 0" 120
  assert_clickhouse_count "$clickhouse_url" "named backend excludes default-only codex session" "SELECT count() FROM ${routed_clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${codex_keyword}') > 0" "0"
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${kimi_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${qwen_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${kiro_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-kiro' AND tool_call_id = 'kiro-tool-${run_stamp}'" 120
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${cursor_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-cursor' AND tool_call_id = 'cursor-tool-${run_stamp}'" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${cursor_sqlite_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'cursor-sqlite' AND tool_call_id = 'cursor-sqlite-tool-${run_stamp}'" 120
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${pi_keyword}') > 0" 120
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.search_postings WHERE term = '${pi_keyword}'" 120
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${hermes_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${hermes_session_keyword}') > 0" 120
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${nac_keyword}') > 0" 120
  ...
  assert_clickhouse_count "$clickhouse_url" "codex tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND tool_call_id = 'codex-tool-${run_stamp}'" "2"
  assert_clickhouse_count "$clickhouse_url" "codex tool request fields" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND tool_call_id = 'codex-tool-${run_stamp}' AND tool_name = 'Read' AND tool_phase = 'request'" "1"
  ...
  assert_clickhouse_count "$clickhouse_url" "codex plain-git tool identity" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND tool_call_id = 'codex-tool-${run_stamp}' AND tool_phase = 'request' AND project_id != '' AND repo_rel_path = 'Cargo.toml' AND worktree_root = '${codex_project_dir}'" "1"
  ...
  clickhouse_scalar "$clickhouse_url" "INSERT INTO ${clickhouse_database}.events SELECT * REPLACE ('' AS project_id, '' AS repo_rel_path, '' AS worktree_root, JSONMergePatch(payload_json, '{\"moraine_tool_io\":{\"project_id\":\"\",\"repo_rel_path\":\"\",\"worktree_root\":\"\"}}') AS payload_json, event_version + 100 AS event_version) FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND event_kind = 'tool_call' AND tool_call_id = 'codex-tool-${run_stamp}'" >/dev/null
  assert_clickhouse_count "$clickhouse_url" "codex legacy tool identity fixture" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND tool_call_id = 'codex-tool-${run_stamp}' AND tool_phase = 'request' AND project_id = '' AND repo_rel_path = '' AND worktree_root = '' AND JSONExtractString(payload_json, 'moraine_tool_io', 'project_id') = ''" "1"
  ...
  clickhouse_scalar "$clickhouse_url" "INSERT INTO ${clickhouse_database}.events SELECT * REPLACE ('${codex_multi_path_session_id}' AS session_id, concat(event_uid, '-multi-path') AS event_uid, 'codex-multi-path-${run_stamp}' AS tool_call_id, '' AS project_id, '' AS repo_rel_path, '' AS worktree_root, JSONMergePatch(payload_json, concat('{\"moraine_tool_io\":{\"input_json\":', toJSONString('{\"path\":\"Cargo.toml\",\"options\":{\"file_path\":\"other.rs\"}}'), ',\"project_id\":\"\",\"repo_rel_path\":\"\",\"worktree_root\":\"\"}}')) AS payload_json, event_version + 200 AS event_version) FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND event_kind = 'tool_call' AND tool_call_id = 'codex-tool-${run_stamp}'" >/dev/null
  clickhouse_scalar "$clickhouse_url" "INSERT INTO ${clickhouse_database}.events SELECT * REPLACE ('${codex_nested_path_session_id}' AS session_id, concat(event_uid, '-nested-path') AS event_uid, 'codex-nested-path-${run_stamp}' AS tool_call_id, '' AS project_id, '' AS repo_rel_path, '' AS worktree_root, JSONMergePatch(payload_json, concat('{\"moraine_tool_io\":{\"input_json\":', toJSONString('{\"options\":{\"path\":\"Cargo.toml\"}}'), ',\"project_id\":\"\",\"repo_rel_path\":\"\",\"worktree_root\":\"\"}}')) AS payload_json, event_version + 300 AS event_version) FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-codex' AND event_kind = 'tool_call' AND tool_call_id = 'codex-tool-${run_stamp}'" >/dev/null
  ...
  assert_clickhouse_count "$clickhouse_url" "claude tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-claude' AND tool_call_id = 'claude-tool-${run_stamp}'" "2"
  assert_clickhouse_count "$clickhouse_url" "claude non-Git tool path normalized" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-claude' AND tool_call_id = 'claude-tool-${run_stamp}' AND tool_phase = 'request' AND startsWith(project_id, 'dir:') AND worktree_root = '${claude_project_dir}' AND repo_rel_path = 'Cargo.toml'" "1"
...
EOF
  ...
  assert_clickhouse_count "$clickhouse_url" "kimi tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-kimi' AND tool_call_id = 'kimi-tool-${run_stamp}'" "2"
  ...
  assert_clickhouse_count "$clickhouse_url" "Qwen canonical title metadata" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE session_id = '${qwen_session_id}' AND event_kind = 'session_meta' AND JSONExtractString(payload_json, 'title') = 'Qwen e2e ${run_stamp}'" "1"
  ...
  assert_clickhouse_count "$clickhouse_url" "Qwen canonical assistant parts" "SELECT uniqExact(tuple(source_offset, event_kind)) FROM ${clickhouse_database}.events FINAL WHERE session_id = '${qwen_session_id}' AND item_id = 'qwen-assistant-${run_stamp}' AND event_kind IN ('reasoning', 'message', 'tool_call')" "3"
  assert_clickhouse_count "$clickhouse_url" "Qwen tool request and response rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'qwen-code' AND tool_call_id IN ('qwen-call-search-${run_stamp}', 'qwen-call-fail-${run_stamp}')" "4"
  assert_clickhouse_count "$clickhouse_url" "Qwen failed tool response" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'qwen-code' AND tool_call_id = 'qwen-call-fail-${run_stamp}' AND tool_phase = 'response' AND tool_error = 1" "1"
  ...
  assert_clickhouse_count "$clickhouse_url" "kiro tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-kiro' AND tool_call_id = 'kiro-tool-${run_stamp}'" "2"
...
EOF
  ...
  assert_clickhouse_count "$clickhouse_url" "cursor tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-cursor' AND tool_call_id = 'cursor-tool-${run_stamp}'" "2"
  ...
  assert_clickhouse_count "$clickhouse_url" "cursor sqlite tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'cursor-sqlite' AND tool_call_id = 'cursor-sqlite-tool-${run_stamp}' AND tool_name = 'edit_file_v2'" "2"
...
PY
  ...
  assert_clickhouse_count "$clickhouse_url" "pi tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-pi' AND tool_call_id = 'pi-tool-${run_stamp}'" "2"
  ...
  assert_clickhouse_count "$clickhouse_url" "hermes trajectory tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-hermes-trajectory' AND tool_call_id = 'hermes-tool-${run_stamp}' AND tool_name = 'weather'" "2"
  ...
  assert_clickhouse_count "$clickhouse_url" "nac credentials absent from search" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE positionCaseInsensitiveUTF8(text_content, '${nac_secret_sentinel}') > 0" "0"
  ...
  assert_clickhouse_count "$clickhouse_url" "nac canonical tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-nac' AND tool_call_id = '${nac_tool_call_id}' AND tool_name = 'search'" "2"
...
PY
  ...
  assert_clickhouse_count "$clickhouse_url" "hermes session tool rows" "SELECT count() FROM ${clickhouse_database}.events FINAL WHERE source_name = 'ci-hermes-session' AND tool_call_id = 'hermes-session-tool-${run_stamp}' AND tool_name = 'shell'" "2"
...
PY
  ...
  wait_for_clickhouse_count "$clickhouse_url" "SELECT count() = 8 FROM ${clickhouse_database}.mcp_event_locator FINAL WHERE session_id = '${nac_replacement_session_id}'" 120
  wait_for_clickhouse_scalar_stable "$clickhouse_url" "SELECT concat(toString(count()), ':', toString(max(event_version))) FROM ${clickhouse_database}.mcp_event_locator FINAL WHERE session_id = '${nac_replacement_session_id}'" 60 5 >/dev/null
  "$python_bin" "$repo_root/scripts/ci/mcp_smoke.py" \
    --moraine "$moraine_bin" \
    --config "$config_path" \
    --working-dir "$nac_project_dir" \
