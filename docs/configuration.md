...
Environment references are accepted only from a config you selected with
`--config`, a config selected through a Moraine config environment override,
or your home config at `~/.moraine/config.toml`. Moraine rejects environment
references in `config/moraine.toml` when that file was discovered only because
it exists in the current repository. To trust that repository file, opt in by
passing `--config ./config/moraine.toml` or setting
`MORAINE_CONFIG=./config/moraine.toml`. Literal ClickHouse values continue to
work in an implicitly discovered repository config.
