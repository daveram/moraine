...
| `harness` | Optional exact, case-sensitive normalized harness filter. Supported values are `antigravity`, `codex`, `claude-code`, `cursor`, `hermes`, `kiro-cli`, `kimi-cli`, `nac`, `opencode`, `pi-coding-agent`, `prime-agent`, and `qwen-code`. |
| `source` | Optional exact, case-sensitive ingest source filter. The default configured values are `antigravity`, `claude`, macOS-only `claude-cowork`, `codex`, `cursor`, `cursor-sqlite`, `hermes`, `kimi-cli`, `kiro`, `omp`, `opencode`, `pi`, `prime-agent`, `prime-agent-subagents`, and `qwen-code`; each server's MCP tool instructions list its actual configured source names. |
